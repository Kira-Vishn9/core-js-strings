...
function formatTime(minutes, seconds) {
  const a = Math.max(0, Math.floor(minutes)).toString().padStart(2, '0');
  const b = Math.max(0, Math.floor(seconds)).toString().padStart(2, '0');
  return `${a}:${b}`;
}
...
function isPalindrome(str) {
  const a = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
